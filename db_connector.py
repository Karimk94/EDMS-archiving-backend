...
def fetch_archived_employees(page=1, page_size=20, search_term=None):
    conn = get_connection()
    ...
    if search_term:
        where_clauses.append("(UPPER(hr.FULLNAME_EN) LIKE :search OR UPPER(hr.FULLNAME_AR) LIKE :search OR hr.EMPNO LIKE :search)")
        params['search'] = f"%{search_term.upper()}%"
    final_where_clause = "WHERE " + " AND ".join(where_clauses) if where_clauses else ""
    try:
        with conn.cursor() as cursor:
            cursor.execute(f"SELECT COUNT(arch.SYSTEM_ID) {base_query} {final_where_clause}", params)
            total_rows = cursor.fetchone()[0]
            fetch_query = f"""
                SELECT arch.SYSTEM_ID, hr.FULLNAME_EN, hr.FULLNAME_AR, hr.EMPNO, hr.DEPARTEMENT, hr.SECTION,
                       stat.NAME_ENGLISH as STATUS_EN, stat.NAME_ARABIC as STATUS_AR
...
def fetch_hr_employees_paginated(search_term="", page=1, page_size=10):
    ...
    search_clause = ""
    if search_term:
        search_clause = " AND (UPPER(hr.FULLNAME_EN) LIKE :search OR UPPER(hr.FULLNAME_AR) LIKE :search OR hr.EMPNO LIKE :search)"
        params['search'] = f"%{search_term.upper()}%"
    try:
        with conn.cursor() as cursor:
            cursor.execute(f"SELECT COUNT(hr.SYSTEM_ID) {base_query} {search_clause}", params)
            total_rows = cursor.fetchone()[0]
            query = f"SELECT SYSTEM_ID, FULLNAME_EN, FULLNAME_AR, EMPNO {base_query} {search_clause} ORDER BY hr.FULLNAME_EN OFFSET :offset ROWS FETCH NEXT :page_size ROWS ONLY"
            params.update({'offset': offset, 'page_size': page_size})
...
def fetch_hr_employee_details(employee_id):
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("SELECT * FROM lkp_hr_employees WHERE SYSTEM_ID = :1", [employee_id])
...
def fetch_statuses():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("SELECT SYSTEM_ID, NAME_ENGLISH, NAME_ARABIC FROM LKP_PTA_EMP_STATUS WHERE DISABLED='0'")
...
def fetch_document_types():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("SELECT SYSTEM_ID, NAME, HAS_EXPIRY FROM LKP_PTA_DOC_TYPES WHERE DISABLED = '0' ORDER BY SYSTEM_ID")
            for row in cursor:
...
def fetch_legislations():
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute("SELECT SYSTEM_ID, NAME FROM LKP_PTA_LEGISL WHERE DISABLED = '0' ORDER BY NAME")
...
def fetch_single_archived_employee(archive_id):
    ...
    try:
        with conn.cursor() as cursor:
            query = "SELECT arch.SYSTEM_ID as ARCHIVE_ID, arch.EMPLOYEE_ID, arch.STATUS_ID, hr.* FROM LKP_PTA_EMP_ARCH arch JOIN lkp_hr_employees hr ON arch.EMPLOYEE_ID = hr.SYSTEM_ID WHERE arch.SYSTEM_ID = :1"
            ...
            doc_query = "SELECT d.SYSTEM_ID, d.DOCNUMBER, d.DOC_TYPE_ID, d.EXPIRY, d.LEGISLATION_ID, dt.NAME as DOC_NAME FROM LKP_PTA_EMP_DOCS d JOIN LKP_PTA_DOC_TYPES dt ON d.DOC_TYPE_ID = dt.SYSTEM_ID WHERE d.PTA_EMP_ARCH_ID = :1 AND d.DISABLED = '0'"
...
def add_employee_archive_with_docs(dst, dms_user, employee_data, documents):
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            archive_query = "INSERT INTO LKP_PTA_EMP_ARCH (SYSTEM_ID, EMPLOYEE_ID, STATUS_ID, DISABLED, LAST_UPDATE) VALUES (:1, :2, :3, '0', SYSDATE)"
            cursor.execute(archive_query, [new_archive_id, employee_data['employee_id'], employee_data['status_id']])
...
def update_archived_employee(dst, dms_user, archive_id, employee_data, new_documents, deleted_doc_ids):
    ...
    try:
        conn.begin()
        with conn.cursor() as cursor:
            update_query = "UPDATE LKP_PTA_EMP_ARCH SET STATUS_ID = :status_id, LAST_UPDATE = SYSDATE WHERE SYSTEM_ID = :archive_id"
            cursor.execute(update_query, {'status_id': employee_data['status_id'], 'archive_id': archive_id})
